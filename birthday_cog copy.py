...
async def generate_birthday_image(display_name: str, template_url=None, output_path="media/birthday_render.png"):
    BASE_DIR = os.getcwd()
    original_url = template_url  # cek apakah custom template dipakai

    # ============================================================
    # Normalisasi URL (khusus Imgur: .jpeg -> .jpg)
    # ============================================================
    if template_url:
    ...
    # ============================================================
    # DOWNLOAD CUSTOM TEMPLATE (kalau ada URL)
    # ============================================================
    base = None
    if template_url:
        try:
            ...
            async with aiohttp.ClientSession(headers=headers) as s:
                async with s.get(template_url, allow_redirects=True) as r:
                    print("DEBUG TEMPLATE URL:", template_url, "STATUS:", r.status)
    ...
    # ============================================================
    if base is None:
        default_path = os.path.join(BASE_DIR, "media", "ultahkos.png")
        print("Pakai default template:", default_path)
        base = Image.open(default_path).convert("RGBA")

    # Resize aman (max 1536 x 1536)
    ...
    # ============================================================
    # CUSTOM TEMPLATE MODE → confetti + BALLOONS
    # ============================================================
    if original_url:
        print("Custom template → add confetti + balloons")

        try:
            confetti_path = os.path.join(BASE_DIR, "media", "confetti.png")
            balloon_path = os.path.join(BASE_DIR, "media", "balloon.png")

            confetti = Image.open(confetti_path).convert("RGBA")
            balloon = Image.open(balloon_path).convert("RGBA")

            print("Confetti size (orig):", confetti.size)
            print("Balloon size (orig):", balloon.size)

            # Confetti full-cover
            confetti = confetti.resize((W, H), Image.LANCZOS)
            base.alpha_composite(confetti, dest=(0, 0))

            # ==================================================
            # Tambahkan BALLOON kecil di kiri & kanan
            # ==================================================
            balloon_count = random.randint(2, 3)
            print("Balloon count:", balloon_count)

            for i in range(balloon_count):

                # --- SCALE KECIL (5% - 9%) ---
                scale = random.uniform(0.05, 0.09)
                bw = max(1, int(balloon.width * scale))
                bh = max(1, int(balloon.height * scale))

                b = balloon.resize((bw, bh), Image.LANCZOS)

                # Rotasi kecil
                angle = random.randint(-10, 10)
                b = b.rotate(angle, expand=True)
                bw, bh = b.size

                # ============================
                # PILIH BENAR-BENAR POJOK
                # ============================
                ...
                print(f"[BALLOON {i+1}] side={side} pos=({pos_x},{pos_y}) size=({bw}x{bh})")

                base.paste(b, (pos_x, pos_y), b)

        except Exception as e:
            print("⚠️ Dekorasi gagal:", e)

        # Simpan hasil custom template
        output_file = os.path.join(BASE_DIR, output_path)
        base.save(output_file)
        print("Saved custom template:", output_file)
        print("============================================\n")
        return output_file
    ...
    output_file = os.path.join(BASE_DIR, output_path)
    base.save(output_file)
    print("Saved default template:", output_file)
    print("============================================\n")
    return output_file
