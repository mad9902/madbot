...
class RobCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.preview_cd = 10        # 10 seconds cooldown for preview

    # ====================================================================
    # Helper: Embed template
    # ====================================================================
    ...
    async def rob_enable(self, ctx):
        ...
        await ctx.send("🟢 Rob system **diaktifkan kembali**.")
    ...
    async def rob(self, ctx, target: discord.Member, confirm=None):
        ...
        if key in self.last_preview and now - self.last_preview[key] < self.preview_cd:
            ...
            return await ctx.send(f"⏳ Tunggu **{rem}s** lagi untuk preview.")
        ...
        v_cash = get_user_cash(self.db, target.id, guild_id)
        ...
        # victim 2h shield
        vshield = get_rob_victim_protect(self.db, guild_id, target.id)
        if vshield > now_ts:
            mins = (vshield - now_ts) // 60
            return await ctx.send(f"🛡 Target aman selama **{mins} menit**.")

        # victim 24h SHOP protection
        shop = get_user_protection(self.db, guild_id, target.id)
        if shop > now_ts:
            hrs = (shop - now_ts) // 3600
            mins = ((shop - now_ts) % 3600) // 60
            return await ctx.send(f"🛡 Target memiliki proteksi: **{hrs} jam {mins} menit**.")

        # compute steal (5–10%)
        percent = random.randint(5, 10)
        steal = int(v_cash * percent / 100)
        punishment = max(1, int(r_cash * 0.10))
...
💀 Risiko gagal: **-{comma(punishment)} coins**
...
"""
    ...
    async def rob_confirm(self, ctx, target: discord.Member):
        ...
        if get_user_protection(self.db, guild_id, target.id) > now_ts:
            return await ctx.send("🛡 Target mengaktifkan proteksi sebelum kamu menyerang.")
        if get_rob_victim_protect(self.db, guild_id, target.id) > now_ts:
        ...
        if random.random() <= chance:
            ...
            set_user_cash(self.db, target.id, guild_id, new_v)

            set_rob_victim_protect(self.db, guild_id, target.id, now_ts + 7200)
...
""",
    ...
    async def buy_protection(self, ctx):
        ...
        active = get_user_protection(self.db, guild, user)
        if active > now_ts:
            left = active - now_ts
            h = left // 3600
            m = (left % 3600) // 60
        ...
        set_user_cash(self.db, user, guild, cash - 500)
        set_user_protection(self.db, guild, user, now_ts + 86400)

        await ctx.send("🛡 Proteksi aktif **24 jam!** Kamu aman dari rob.")

    # ====================================================================
    # STATUS (bebas di mana saja)
    # ====================================================================
    ...
    async def robstatus(self, ctx):
        ...
        shop = get_user_protection(self.db, guild, user)
        shield = get_rob_victim_protect(self.db, guild, user)
